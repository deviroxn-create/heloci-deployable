 ...
 */
interface TemplateRepair {
  name: string; // Planner key: audience.event.channel
  eventName: string; // Internal event name
  channel: "email" | "internal" | "telegram";
  subject: string;
  title: string;
  htmlContent: string;
  plainTextContent: string;
  repairType: "CREATE" | "CREATE_AUDIENCE_KEY"; // Classification from Phase 5H.7
  workflow: string; // Which workflow this serves
}

const REPAIRS: TemplateRepair[] = [
  ...
  // These already have generic fallback templates in DB, but we're creating audience-specific versions
...
async function repairTemplateRegistry() {
  ...
  const audiencePrefixedRecords = await prisma.notificationTemplate.count({
    where: {
      name: {
        contains: "."
      }
