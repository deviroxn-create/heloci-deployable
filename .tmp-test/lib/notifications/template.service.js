...
exports.buildNotificationTemplateWhere = buildNotificationTemplateWhere;
exports.buildTemplateHtml = buildTemplateHtml;
exports.buildTemplatePlainText = buildTemplatePlainText;
exports.getPublishedTemplate = getPublishedTemplate;
exports.getSettingsTemplateMap = getSettingsTemplateMap;
exports.saveNotificationTemplate = saveNotificationTemplate;
exports.syncTemplatesFromSettings = syncTemplatesFromSettings;
...
function renderTemplatePreview(template, payload) {
    return renderTemplate(template, payload);
}
function buildNotificationTemplateWhere(input) {
    const where = {};
    if (input.eventName) {
...
function buildTemplateHtml(templateBody) {
    return `<div>${templateBody.replace(/\n/g, "<br />")}</div>`;
}
function buildTemplatePlainText(templateBody) {
    return templateBody;
}
async function getPublishedTemplate(eventName, channel, locale = "en") {
    const template = await client_1.prisma.notificationTemplate.findFirst({
    ...
    const fallback = exports.defaultTemplates[eventName];
    return {
        id: "default",
...
async function getSettingsTemplateMap() {
    ...
    const result = { ...exports.defaultTemplates };
    for (const template of templates) {
        const draft = template.plainText || template.html.replace(/<[^>]+>/g, " ");
        result[template.eventName] = {
...
async function saveNotificationTemplate(input) {
    const existing = await client_1.prisma.notificationTemplate.findFirst({
        where: {
            name: input.name,
    ...
    if (existing) {
        return client_1.prisma.notificationTemplate.update({
    ...
    return client_1.prisma.notificationTemplate.create({
        data: {
            name: input.name,
            eventName: input.eventName,
            channel: input.channel,
...
async function syncTemplatesFromSettings(templates) {
